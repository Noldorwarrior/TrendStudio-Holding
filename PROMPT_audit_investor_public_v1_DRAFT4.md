...
- **B** — математика и логика (Revenue, Cost, P&L→CF→BS, Debt/Covenants, Waterfall+Investor Rights, Cap Table, DCF, MC, Tax, WC, Unit Economics, Accounting Identity, Logic Tests 4-типа, **Precision & Rounding**, **Arguments Coherence**, **Logical Fallacies**, **Red-Team**)
...
- **G** — **Технический аудит инфраструктуры аудита** (архитектура `audit_scripts/`, качество кода, tech debt, воспроизводимость)
...
### 1. `audit_public_v1_findings.xlsx` (11 листов)
...
9. **Script Results** — сводка 28 скриптов
10. **Arguments Ranking** ⚠️ НОВЫЙ — сильнейшие/слабейшие аргументы
11. **Red-Team Attack** ⚠️ НОВЫЙ — 10 самых неприятных вопросов VC + ответы
...
- [ ] **Технический аудит `audit_scripts/`** (architecture + quality + reproducibility + tech debt)
...
4 файла (findings 11 листов + exec summary 3–5 стр + red flag memo 1 стр + remediation roadmap xlsx) + `audit_scripts/` (28 скриптов, сам себя проверяющих через блок G). Уровень **DD-grade + Red-team battle-tested**.
