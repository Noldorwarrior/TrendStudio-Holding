# ПРОМТ: HTML-лендинг ТрендСтудио Холдинг (v1.2)

**Статус:** v1.2 — третья итерация промта с учётом 8 дополнительных решений пользователя по тиру симуляторов, marquee-схеме, стилю портретов и раскладке по секциям.
**Базируется на:** `Промт_HTML_лендинг_Холдинг_v1.1.md` (2026-04-19) + архитектуре `Промт_HTML_лендинг_ЦПИКП_v5.2.md`.
**Дата:** 2026-04-19.
...
**Формат доставки промта:** **4 файла** — этот `.md` + `canon_holding_base.json` (18 блоков) + `canon_holding_extended.json` (23 блока, +1 относительно v1.0) + `Gemini_TZ_images_v1.0.md` (техзадание на 20 ключевых изображений).
...
|---|---------|--------------------------|
| 1 | Все 13 симуляторов сохранены | §5 переструктурирован: 3 marquee + 7 hero + 3 standard tier |
| 2 | Marquee-тир (3 шт.) | MC @ s03 (hook), Pipeline Builder @ s12 (центр), LP Sizer @ s20 (перед CTA) — см. §3.1 + §5.1 |
| 3 | Hero-тир расширен до 7 | + Stress Test (gauge cluster 3D) + Distribution Mix (Sankey + частицы) + IP Value (waterfall + force-graph) к исходным MC/Pipeline/LP Sizer/Box-Office |
| 4 | Stress Test: 3D gauge cluster | Three.js torus/гауджи + цепная анимация по 7 проектам, радар пересчёт IRR/MOIC/DPI в real-time (§5.2) |
| 5 | Distribution Mix: Sankey + animated particles | D3 Sankey (box-office → theatrical/OTT/TV/int'l/merch), процентные слайдеры меняют потоки (§5.2) |
| 6 | IP Value: waterfall + force-graph | D3 waterfall по окнам + force-граф прав (theatrical/OTT/int'l/TV/merch/продолжения), drill-down по нодам (§5.2) |
| 7 | Marquee-стандарты анимации | Все 3 marquee имеют cinematic camera-reveal (GSAP ScrollTrigger pin + scrub), длительность 4-8 сек — criterion 73 (§7.1) |
| 8 | Portrait style «Тени заката» | Silhouette + ember rim-light для 9 портретов (5 team + 4 advisory): silhouette 3/4-pose, ember rim #C77B3A сверху-сзади, navy #0D1117 фон, grain-плёнка (§1.7.3 + Приложение В) |

Остальные решения из v1.0 и v1.1 (якоря, 25 секций, 22 визуализации, аудитории LP/Partners/Gov, мобильность, П5 32/32 + М4 7/7, палитра Nolan/Dune, 3-слойный фон, 20 Gemini-слотов) — **без изменений**, детали сохранены.

---

## §0.1. Карта родословной и отличий от ЦПИКП v5.2

| Раздел | ЦПИКП v5.2 (образец) | Холдинг v1.2 (этот промт) |
|--------|-----------------------|----------------------------|
...
### §1.7.3. Portrait style «Тени заката» (Silhouette + ember rim-light) — v1.2 ФИКСИРОВАН

Пользователь выбрал единый стиль портретов для 9 слотов (5 team + 4 advisory): **«Тени заката»** — силуэт в 3/4-позе с тёплой ember-подсветкой с верхне-заднего плана.

**Эстетика-референс:**
- Кристофер Нолан, «Tenet», «Oppenheimer» — контражур, работа со светом как с героем сцены.
- «Бегущий по лезвию 2049» (Денис Вильнёв / Роджер Дикинс) — силуэт на монохромном glow-фоне.
- Дэвид Финчер, «Mank» — портрет как архетип, не как идентификация.

**Технические требования стиля (обязательны в Gemini TZ):**

| Параметр | Значение |
|---|---|
| Композиция | 3/4 поворот, поясной план (от груди до головы) |
| Источник света | Rim-light сверху-сзади, тёплый ember #C77B3A, интенсивность «ударная» |
| Фон | Тёмный navy-to-black gradient #0D1117 → #050508, равномерный без объектов |
| Силуэт | Читаемый контур, но **без детализации черт лица** (лицо в тени / затемнено rim-light'ом) |
| Тёплый spill | Лёгкий gold-ember bleed на плечо/висок/скулу от rim-light |
| Film grain | Средняя интенсивность (subtle, как 35mm Kodak 500T push +1) |
| Chromatic aberration | Минимальная на краях, только для ощущения cinematic lens |
| Format | 1:1 (square) для grid, 180 KB после JPG Q75 |
| Avoid | Реалистичная идентифицируемая личность, яркие cyan/electric тона, плоская illustration-стилистика, corporate headshot |

**Почему именно такой стиль (обоснование):**
1. **Лицо в тени** снимает этический и юридический риск «real person» при том, что ТЗ запрещает реалистичные портреты несуществующих людей.
2. **Архетипичность** — инвестор читает роль, а не лицо (CEO, Producer Lead, CFO); имена конкретных людей проставляются в caption, а не в изображении.
3. **Cohesion с палитрой** — ember #C77B3A и gold #D4AF37 уже являются акцентами палитры v1.1 (§1.3), портреты органично встраиваются в dark cinematic narrative без визуального диссонанса.
4. **Production-grade aesthetic** — подчёркивает prestige-film позиционирование холдинга (а не стартап / tech).

**9 слотов портретов (будет зафиксировано в Gemini TZ, Приложение В):**

| # | Slot | Роль | Дополнительный атрибут (для distinction) |
|---|------|------|------------------------------------------|
| 1 | `team/01_ceo.jpg` | CEO | Очертание строгого costume-silhouette |
| 2 | `team/02_producer_lead.jpg` | Producer Lead | Силуэт с раскрытым scripted notebook |
| 3 | `team/03_cfo.jpg` | CFO | Классический business-silhouette |
| 4 | `team/04_head_distribution.jpg` | Head of Distribution | Силуэт у окна / horizon-line фона |
| 5 | `team/05_creative_director.jpg` | Creative Director | Силуэт с film-slate в руке |
| 6 | `advisory/01_industry_veteran.jpg` | Industry Veteran | Более «взрослый» pose, раздумчивый профиль |
| 7 | `advisory/02_finance_advisor.jpg` | Finance Advisor | Строгий business-silhouette, прямая спина |
| 8 | `advisory/03_distribution_advisor.jpg` | Distribution Advisor | Силуэт-вполоборота, жестикуляция |
| 9 | `advisory/04_international_advisor.jpg` | International Advisor | Силуэт у globe / map-projection на фоне |

**Prompt-шаблон для Gemini (будет детализирован в Приложении В):**

> Cinematic silhouette portrait, 3/4 three-quarter pose, chest-up framing. Figure in dark costume, **face obscured by deep shadow** (features not identifiable). Strong warm ember rim-light (#C77B3A, hex warm amber) from upper-back, creating a bright halo around the silhouette's head and shoulder. Subtle warm gold spill (#D4AF37) on cheekbone and temple. Background: deep navy-to-black radial gradient (#0D1117 fading to #050508), no objects, no clutter. Subtle 35mm film grain, minimal chromatic aberration at edges. Aesthetic reference: Christopher Nolan + Denis Villeneuve + David Fincher prestige-film cinematography. Role attribute: <role-specific addition from slot table>. Avoid: identifiable facial features, bright saturated colors, flat illustration style, corporate headshot aesthetic, cyan/electric tones.

### §1.8. Структура каталога v1.2

Финальный deliverable — один `.html` файл, но в процессе работы структура такая:

```
/Холдинг/
├── Промт_HTML_лендинг_Холдинг_v1.2.md       ← этот файл
├── Gemini_TZ_images_v1.0.md                  ← ТЗ для Gemini (создаётся после утверждения v1.2)
...
### §3.1. Базовые 21 секция (v1.2 — размечены marquee-слоты M1/M2/M3)

Колонка «T» — тир симулятора/визуализации: **M** = Marquee (центр внимания), **H** = Hero (расширенная), **S** = Standard (базовая), **—** = не симулятор.

| # | Секция | T | Основная цель | Источник текста | Главная визуализация |
|---|--------|---|---------------|-----------------|----------------------|
...
| **3** | **Monte Carlo (MARQUEE M1 — hook)** | **M** | **«Масштаб анализа»: ранний wow-эффект** | `monte_carlo` + `narrative.market_hook` | **Three.js 3D scatter 1000 точек + p50/p25/p75 overlay + cinematic fly-through** |
| 4 | **Financial Core** | S1 | IRR / MOIC / TVPI | `returns` | Time-series 27 точек × 4 сценария + scrub + IRR/MOIC Calculator inline |
| 5 | **Our Model** | — | Business model | `narrative.model` | Force-graph (studios / distribution / IP / fund) |
| 6 | **Benchmark** | — | Сравнение с аналогами | `benchmark` | Radar 8 осей × 5 студий |
| 7 | **Market Context** | — | Почему сейчас | `narrative.market` | Choropleth РФ (15 регионов) + banner `market_context.jpg` |
| 8 | **Box-Office Sensitivity** | H1 | Как меняются сборы от параметров релиза | `pipeline.projects` + `weekly_box_office` | Heatmap 16 недель × 7 проектов + Box-Office Sensitivity simulator |
| 9 | **Capital Structure** | — | LP / GP / waterfall | `deal_structure` | Sankey cash-flow |
| 10 | **Cash Flow Timeline** | — | Capital calls / distributions | `time_series.cashflow` | Waterfall + кумулятивная кривая |
| 11 | **Release Window Optimizer** | S5 | Окно × экран × конкуренция | `release_windows` | Heatmap 5 × 4 + Release Window simulator |
| **12** | **Pipeline Builder (MARQUEE M2 — центр)** | **M** | **«Управляй портфелем» (агентность LP)** | `pipeline.projects` + `ip_portfolio` + `canon.images.projects` | **Three.js 3D board 7×4 drag-n-drop + live IRR/NDP/DPI пересчёт + Sankey cash-flow** |
| 13 | **Geography** | — | Региональные налог. льготы | `regions` + `tax_credits` | Choropleth + bubble overlay |
| 14 | **Scenarios + Break-even** | S2+S3 | Base / Bull / Bear / Stress + BE-slider | `scenarios` | Scenario-switcher (морф графиков) + Break-even Slider |
| 15 | **Stress Test Matrix** | H2 | Шоки × уровни | `stress_tests` | Three.js gauge cluster (7 торов) + цепная анимация + радар IRR/MOIC/DPI |
| 16 | **Roadmap** | — | 7-летний план | `roadmap` | Horizontal timeline + milestones reveal |
| 17 | **Distribution Mix** | H3 | Каналы (theatrical/OTT/TV/int'l/merch) | `distribution` | D3 Sankey + animated particles + 5 слайдеров |
| 18 | **Theatrical vs OTT + Tax** | S6 | Гибрид vs чистый театр | `distribution` + `tax_credits` | Break-even matrix + ROI delta bar |
| 19 | **IP Value Estimator** | H4 | Оценка стоимости IP | `ip_portfolio` | D3 waterfall + force-граф прав (6 нод) + drill-down modal |
| **20** | **LP Commitment Sizer (MARQUEE M3 — перед CTA)** | **M** | **«Ваш личный сценарий»** | `deal_structure` + `term_sheet` | **Radar 5 осей + тикет-slider 50-1000 млн ₽ + live net IRR/MOIC/DPI** |
| 21 | **CTA / Contact** | — | Contact + NDA + next steps | `narrative.cta` | Form + calendar embed |

**Порядок нарратива (v1.2):** Hero → Thesis → **MC hook** → Financial → Model → Benchmark → Market → **Box-Office H1** → Capital → Cashflow → Windows → **Pipeline Builder MARQUEE** → Geography → Scenarios+BE → **Stress H2** → Roadmap → **Distribution H3** → OTT+Tax → **IP Value H4** → **LP Sizer MARQUEE** → CTA.

Секции 12 и 20 (Marquee M2 и M3) — full-screen pin на desktop (GSAP ScrollTrigger pin + scrub), секция 3 (Marquee M1) — пинна 4-6 сек с cinematic camera reveal.
...
## §5. 13 симуляторов (интерактивные калькуляторы) — 3-тировая схема (v1.2)

В `canon.simulators` — 13 формул. Агент реализует **все 13**, но по 3 тирам визуализации/вовлечённости.

### §5.1. Marquee-тир (3 шт.) — «звёзды лендинга»

Центральные точки внимания. Обязаны иметь: cinematic camera-reveal (GSAP ScrollTrigger pin + scrub, 4-8 сек), интро-анимацию знакомства («как пользоваться»), целевое время взаимодействия 60-180 сек, exit-tween, H1-уровень заголовка, полноэкранная секция на desktop.

| # | Симулятор | Слот | Роль в нарративе | Ключевая визуализация | Время LP |
|---|-----------|------|-------------------|------------------------|----------|
| **M1** | **Monte Carlo 1000 runs** | **s03 (hook)** | «Масштаб и научность анализа» — ранний wow-эффект сразу после thesis | Three.js 3D scatter 1000 точек p(IRR) + p50/p25/p75 overlay + cinematic camera fly-through | 30-60 сек |
| **M2** | **Pipeline Builder** | **s12 (центр)** | «Управляй портфелем» — центр разбора модели, агентность LP | Three.js 3D board 7 проектов × 4 стадии drag-n-drop + live IRR/NDP/DPI пересчёт + Sankey cash-flow | 120-180 сек |
| **M3** | **LP Commitment Sizer** | **s20 (перед CTA)** | «Ваш личный сценарий» — последний штрих перед call-to-action | Radar chart (risk/return/horizon/liquidity/strategic) + slider тикета 50-1000 млн ₽ + live net IRR / MOIC / DPI | 60-90 сек |

### §5.2. Hero-тир (4 шт.) — «богатые интерактивы»

Расширенная визуализация с 3D или сложным D3, но без полного cinematic reveal. Целевое время 30-90 сек. Half-screen на desktop.

| # | Симулятор | Слот | Визуализация |
|---|-----------|------|--------------|
| H1 | **Box-Office Sensitivity** | s08 | Heatmap 16 недель × 7 проектов с cell-hover tooltip, GSAP-morph при изменении opening/drop/screens |
| H2 | **Stress Test Matrix** | s15 | Three.js gauge cluster (7 торов = 7 проектов), цепная анимация при применении шока, радар IRR/MOIC/DPI пересчёт real-time |
| H3 | **Distribution Mix** | s17 | D3 Sankey (box-office → theatrical/OTT/TV/int'l/merch) + animated particles по рёбрам + 5 процентных слайдеров меняют потоки |
| H4 | **IP Value Estimator** | s19 | D3 waterfall по окнам + force-граф прав (6 нод: theatrical/OTT/int'l/TV/merch/продолжения) + drill-down modal по ноде |

### §5.3. Standard-тир (6 шт.) — «функциональные калькуляторы»

Базовая 2D-визуализация (SVG/Canvas без 3D). Inline-расположение в соответствующих секциях, целевое время 15-45 сек.

| # | Симулятор | Слот | Визуализация |
|---|-----------|------|--------------|
| S1 | **IRR / MOIC Calculator** | s04 | Time-series + cumulative curve + KPI-блок |
| S2 | **Scenario Switcher** | сквозной (header) | Lookup `canon.scenarios` → морф графиков §7 (global state) |
| S3 | **Break-even Slider** | s14 | Break-even marker на 2D-графике |
| S4 | **Term Sheet Simulator** | s22 | Waterfall chart LP/GP split |
| S5 | **Release Window Optimizer** | s11 | Heatmap 5 window × 4 screens |
| S6 | **Theatrical vs OTT + Genre/Tax Optimizer** | s18 | Break-even matrix + ROI delta bar |

### §5.4. Общие требования для всех 13

**Каждый симулятор обязан:**
1. `<fieldset>` входов + `<div>` результата (aria-live="polite").
2. Live-update (debounced 100 ms).
3. URL hash state (без localStorage).
4. Mobile-friendly (touch-targets ≥ 44 px).
5. Reset-кнопка (восстановление дефолтов).
6. Tooltip «как пользоваться» при первом фокусе (показ один раз, dismissable).

**Дополнительно для Marquee (M1-M3):**
7. Cinematic reveal через GSAP ScrollTrigger (pin + scrub, 4-8 сек).
8. Intro-tour (GSAP timeline, 3-5 шагов с подсветкой элементов).
9. Exit-tween при уходе секции (fade + blur).
10. Background parallax L1+L2 (film-grain ambient particles).

**Дополнительно для Hero (H1-H4):**
11. Initial reveal через staggerFadeIn за 2 сек.
12. Ambient idle-animation (subtle breathing 3-5 сек loop).
...
### §7.1. Критерии приёмки (60 базовых + 14 Holding-специфичных = 74)

Базовые 60 наследуются из ЦПИКП v5.2. Ниже 14 Holding-специфичных (в v1.2 +2 относительно v1.1):
...
**73. Marquee cinematic reveal (v1.2 — НОВОЕ).** 3 marquee-симулятора (M1 MC @ s03, M2 Pipeline Builder @ s12, M3 LP Sizer @ s20) имеют обязательный GSAP ScrollTrigger pin+scrub cinematic reveal длительностью 4-8 сек, intro-tour 3-5 шагов, exit-tween (fade+blur), и ambient L1+L2 background parallax. Проверяется Self-check: `document.querySelectorAll('[data-tier="marquee"][data-cinematic-reveal="true"]').length === 3`.

**74. Portrait style «Тени заката» (v1.2 — НОВОЕ).** Все 9 портретов (5 team + 4 advisory) выполнены в едином стиле §1.7.3: силуэт 3/4 + ember rim-light #C77B3A + navy-to-black фон + face obscured. Отсутствие идентифицируемых лиц — обязательно (ethical/legal risk). Проверяется manual-review после Gemini-генерации до base64-embedding и фиксируется в `canon.images.team[*].style_signature === "shadows_of_sunset_v1"`.
...
```html
<!-- ================================================================
VERIFICATION REPORT — Промт v1.2 Лендинг ТрендСтудио Холдинг
...
М4.2 ✓ Критерии приёмки: 74/74 выполнены
...
Coverage:
  ...
  - N_viz_types = {{}}/22
  - N_simulators = 13/13 (3 marquee + 4 hero + 6 standard)
  - N_marquee_cinematic = 3/3 (M1 s03 + M2 s12 + M3 s20)
  - N_images = 20/20 (including 9 «Тени заката» portraits)
...
## Приложение А. Шпаргалка для агента v1.2
...
Финальный workflow перехода от промта v1.2 к HTML:

1. **Утверждение промта v1.2** пользователем (после прочтения этого файла).
...
6. **Генерация `canon_holding_extended.json`** (23 блока) — визуализации + interactivity rules + simulators + **`canon.images` registry** (с путями `images/01_...jpg`, без base64) + `style_signature: "shadows_of_sunset_v1"` в `canon.images.team[*]` и `canon.images.advisory[*]` (v1.2).
7. **Sanity-check** по §2.4 (якоря + images registry + marquee tier 3/3 + portrait style signature 9/9).
8. **Применение промта v1.2** → Claude генерирует HTML по §9 (7 фаз).
9. В Фазе 6 агент inline-pack base64 изображений → финальный `.html`.
10. **П5 32/32 + М4 7/7 отчёт** — в комментарии HTML (по 74 критериям).

---

## Приложение В. Gemini TZ — черновик (будет вынесен в отдельный файл после утверждения v1.2)

После утверждения этого промта v1.2 создаётся отдельный файл `Gemini_TZ_images_v1.0.md` со следующей структурой:
...
2. **20 индивидуальных промтов** — по одному на каждый слот:
   ...
   - **team/01-05 + advisory/01-04 — 9 портретов в стиле «Тени заката» (v1.2, см. §1.7.3):** силуэт 3/4 + ember rim-light #C77B3A + navy-to-black фон + лицо в тени (face obscured). Единый prompt-шаблон (§1.7.3) + role-specific addition для каждого из 9 слотов (CEO, Producer Lead, CFO, Head of Distribution, Creative Director, Industry Veteran, Finance Advisor, Distribution Advisor, International Advisor). Каждый портрет помечается в canon `style_signature: "shadows_of_sunset_v1"` для верификации критерия №74.
...
5. **QA-чек-лист** после получения от Gemini: 20/20 файлов, имена совпадают, разрешения совпадают, средний вес ≤ 350 KB, no text/watermarks, стилевая когерентность 5/5 (subjective review). **Для 9 портретов дополнительно: manual verification «Тени заката» — лица должны быть затемнены, ember rim-light читаем, идентифицируемых реальных людей нет (v1.2).**

Этот черновик становится файлом `Gemini_TZ_images_v1.0.md` **на следующем шаге после утверждения v1.2** — в том же каталоге `/Холдинг/`.
...
## Приложение Г.2. Diff v1.1 → v1.2 (сводка для верификации №24, v1.2)

| Раздел | v1.1 | v1.2 |
|--------|------|------|
| §0 Change log | 8 строк v1.0→v1.1 | **8 строк v1.1→v1.2** (marquee-тир, hero-расширение, «Тени заката») |
| §3.1 sections table | 21 секция без tier-разметки | 21 секция с **колонкой T (M/H/S/—)** и reordering под marquee на s03/s12/s20 |
| §5 simulators | плоский список 13 | **3-тировая схема** §5.1 Marquee (3) / §5.2 Hero (4) / §5.3 Standard (6) |
| §5.4 common requirements | 6 пунктов для всех 13 | 6 общих + **4 additional для Marquee** (cinematic reveal + intro-tour + exit-tween + ambient parallax) + **2 для Hero** (staggerFadeIn + idle-breathing) |
| §1.7.3 portrait style | отсутствует | **§1.7.3 НОВАЯ** — «Тени заката»: силуэт 3/4 + ember rim-light #C77B3A + face obscured + детализация 9 слотов |
| Критериев приёмки | 72 (60 + 12 Holding) | **74** (60 + 14 Holding: добавлены **73 Marquee cinematic reveal** и **74 Portrait style «Тени заката»**) |
| Marquee simulators на лендинге | не выделены | 3 шт. с pin+scrub ScrollTrigger 4-8 сек: **M1 MC @ s03, M2 Pipeline Builder @ s12, M3 LP Sizer @ s20** |
| Hero simulators | 3 (MC, Pipeline Builder, LP Sizer) | **4** (Box-Office, Stress Test, Distribution Mix, IP Value) — MC/Pipeline/LP Sizer ушли в Marquee |
| Style signature в canon | отсутствует | **`canon.images.team[*].style_signature === "shadows_of_sunset_v1"`** для верификации №74 |
| Приложение В (Gemini TZ) | 9 портретов без стиля | **9 портретов в «Тени заката»** + role-specific addition + manual-verification checkbox |
| VERIFICATION REPORT | 72 criteria, 13 simulators | **74 criteria**, **13 simulators (3M+4H+6S)**, **N_marquee_cinematic 3/3**, 9 portraits style-tagged |
| Приложение Г | Г (v1.0→v1.1) | Г + **Г.2 (v1.1→v1.2)** ← эта таблица |

---

**Конец промта v1.2.**

**Статус:** драфт v1.2 с учётом 13 решений пользователя текущей итерации. После подтверждения — **полная верификация П5 «Максимум» 32/32 + М4 Презентационная 7/7** с последующим отчётом в `/Холдинг/Верификация_Промт_v1.2_П5_М4.md`, затем переход к `Gemini_TZ_images_v1.0.md` (Приложение В в расширении).
**Следующий шаг:** запуск верификации v1.2 (задача #6 в task list).
